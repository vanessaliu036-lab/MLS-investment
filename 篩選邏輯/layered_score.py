...
# ══════════════════════════════════════════════════════════
# 四道結構閘門（V3 2026-08-13）：淘汰需四道同時成立，不是任兩項投票制。
# 舊版(V2)用「>=2 不同類」湊分,靠 _FAIL_CATEGORY 把同義失效項折成同一類防重複計分；
# V3 把四道閘門本身定義成互斥面向(價格/資金/量價/反彈)，門檻直接要求四道全中，
# _FAIL_CATEGORY 那層映射已經沒有存在意義(其字面 key 也對不上下面 failure_gates()
# 實際回傳的名稱，早已是死碼)，一併移除，不留一份會誤導人的舊映射表。
