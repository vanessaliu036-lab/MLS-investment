...
def _all_sector_names():
    """
    從 config.SECTOR_MAP 動態取得完整族群清單(去重、保序)。
    不寫死名單:日後在 config 增減族群,補齊邏輯自動跟著變。
    """
    seen, names = set(), []
    for _sec, _t in C.SECTOR_MAP.values():
        if _sec and _sec not in seen:
            seen.add(_sec)
            names.append(_sec)
    return names
...
def compute_sector_flow(snaps):
    ...
    # ── [補齊固定族群] 保證永遠回傳完整族群數,抓不到的補佔位 ──
    # 根因:某批 snapshots 失敗時,該族群成員全缺席 → 該族群整組不進 sectors
    #      → 前端 10 格變 8 格。這裡用 SECTOR_MAP 的完整族群清單回填缺席者。
    # 佔位 status="no_data":前端顯示灰格「資料未取得」,而非消失或假裝為 0。
    _present = {s["name"] for s in sectors}
    for _name in _all_sector_names():
        if _name not in _present:
            sectors.append({
                "name": _name, "type": "attack",
                "pct": 0.0, "amount_100m": 0.0, "amount_share": 0.0,
                "flow_score": 0.0, "locked": False, "members": [],
                "status": "no_data",   # ← 缺席標記,前端據此灰格顯示
            })
...
def eval_stock(s, locked_sectors, *, sector_median=0.0, market_pct=0.0,
    ...
    # Shioaji 的即時 quote.volume_ratio 在訂閱模式常回傳 0；
    # 盤中評分實際使用的是成交量對近 5 日均量的 tnvr，兩者必須統一。
    tnvr_val = scoring.tnvr(s.get("total_volume"), _avg5_volume(s["code"]))
    effective_volume_ratio = (tnvr_val if tnvr_val is not None else
                              (s.get("volume_ratio") or 0))
    s = {**s, "volume_ratio": effective_volume_ratio}
    ...
    if risk and s["change_rate"] < 0:
        ...
        rules = rules + ["邊拉邊賣⚠"]
    elif entry_n >= 2 and score >= _entry_min() + 15 and bs_pass:
        action, ec = "buy", "entry_high"
    elif entry_n >= 1 and score >= _entry_min() and bs_pass:
        action, ec = "buy", "entry"
    elif entry_n >= 1 and score >= _entry_min() and not bs_pass:
        action, ec = "watch", "potential"      # 分數夠但BS濾網未過→只觀察不開槍
        rules = rules + ["BS未過"]
    ...
    return {
        "action": action,
        "ai_score": score,
        "factors": factors, "penalties": penalties, "factor_notes": factor_notes,
        "chip": chip or {},
        "tnvr": tnvr_val,
        "bs": bs_detail.get("bs"), "bs_pass": bs_pass,
        "bs_recent": bs_detail.get("recent_bs_approx"),
...
def build_state(watchlist_codes=None):
    ...
    raw_snaps = (broker.raw_buffer_snapshots()
                 if getattr(C, "USE_SUBSCRIBE", True)
                 else broker.batch_snapshots(codes))

    # 硬過濾:流動性(動態閾值:開盤 30 分鐘內放寬,其他時段嚴格)
    _vol_threshold = (C.min_volume_lots() if hasattr(C, "min_volume_lots") else C.MIN_VOLUME_LOTS) * 1000
    snaps = [s for s in raw_snaps
             if (s["total_volume"] or 0) >= _vol_threshold]
    # 盤中保底:Shioaji 已有即時快照但量能門檻將整池濾空時,
    # 仍用已收到的即時資料計算族群/資金流,避免整頁空白。
    if raw_snaps and len(snaps) < 5:
        print(f"[engine] 量能門檻 {_vol_threshold} 僅保留 {len(snaps)} 檔,改用 {len(raw_snaps)} 檔即時資料避免盤中空白")
        snaps = raw_snaps

    # ① 資金流入板塊
    # 資金流向與個股篩選完全解耦:族群流向一律使用原始即時快照,
    # 不受量能門檻、W條件或評分過濾影響。
    sectors = compute_sector_flow(raw_snaps)
    # 個股評分資料是另一條路徑;補回族群歸屬供 eval_stock 使用。
    # compute_sector_flow 只會標記 raw_snaps,不會標記被量能過濾後的 snaps。
    sector_map = {
        m["code"]: sec["name"]
        for sec in sectors
    ...
    for s in snaps:
        if "sector" not in s:
            sec, stype = C.SECTOR_MAP.get(s["code"], (None, None))
            s["sector"] = sector_map.get(s["code"], sec)
            s["sector_type"] = stype
    print(f"[engine] snaps 有 sector 的數量: {sum(1 for s in snaps if 'sector' in s)} / {len(snaps)}")
    ...
    try:
        import money_health
        sec_health, _vc = money_health.annotate(snaps, sectors, market_pct)
        _hmap = {s["code"]: s for s in snaps}
        for row in table:
            hs = _hmap.get(row["code"])
            if hs and hs.get("_health"):
                row["health"] = hs["_health"]
                row["triangulation"] = hs.get("_tri")
                # money_health 可能沒有籌碼資料；不可用空值覆蓋 scoring 的真實快取。
                row["chip"] = hs.get("_chip") or row.get("chip") or {}
                row["doc_strategy"] = hs.get("_doc_strategy")  # 文件四大條件(A/B對比)
    except Exception as _e:
        print(f"[plugin/money_health] 跳過:{_e}")

    # 插件:文件四大策略(A/B 對比,失敗不影響主流程)
    try:
        import strategy_doc
        idx = broker.index_snapshot()
        mkb = broker.daily_kbars("TSE001", days=25) if hasattr(broker, "daily_kbars") else []
        mclose = idx.get("index")
        mma20 = (sum(k["close"] for k in mkb[-20:]) / 20) if len(mkb) >= 20 else None
        strategy_doc.annotate(snaps, market_pct=market_pct,
                              market_close=mclose, market_ma20=mma20,
                              kbar_fn=lambda c: broker.daily_kbars(c, days=25))
        _dmap = {s["code"]: s for s in snaps}
        for row in table:
            hs = _dmap.get(row["code"])
            if hs and hs.get("_doc_strategy"):
                row["doc_strategy"] = hs["_doc_strategy"]
    except Exception as _e:
        print(f"[plugin/strategy_doc] 跳過:{_e}")
    ...
    return {
        ...
        "stocks": table[:60],
        "raw_count": len(raw_snaps),
        "filtered_count": len(snaps),
