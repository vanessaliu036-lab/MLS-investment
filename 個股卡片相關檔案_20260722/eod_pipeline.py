...
# ════════════════════════════════════════════════════════
def fetch_eod_snaps():
    """收盤後抓官方 TWSE/TPEx 個股收盤；券商只作官方端點失敗時備援。"""
    try:
        import eod_source
        snaps = eod_source.eod_snaps(codes=list(C.UNIVERSE))
        if snaps and len(snaps) >= max(5, len(C.UNIVERSE) // 2):
            source = {s.get("source") for s in snaps}
            print(f"[eod] 官方個股收盤 {len(snaps)}/{len(C.UNIVERSE)} 檔,來源={source}")
        else:
            snaps = []
    except Exception as e:
        print(f"[eod] 官方個股收盤取得失敗:{e}")
        snaps = []

    if not snaps:
        import broker
        snaps = broker.batch_snapshots(list(C.UNIVERSE))
        print(f"[eod] 官方不足,券商備援 {len(snaps)}/{len(C.UNIVERSE)} 檔")
    for s in snaps:
