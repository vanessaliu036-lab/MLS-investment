...
BASE_FIELDS = ("inst_net_20d_lots", "inst_streak",
               "big_holder_pct", "big_holder_trend")


def _quality(data, default="finmind_basic"):
    """Do not label an empty/failed source as connected data."""
    if (data or {}).get("source_type") == "official":
        return "official"
    if any((data or {}).get(k) is not None for k in RICH_FIELDS):
        return "premium"
    if any((data or {}).get(k) is not None for k in BASE_FIELDS):
        return default
    return "unavailable"
        ...
        try:
            ...
            if data:
                ...
                for k, v in base.items():
                    data.setdefault(k, v)
                return data, _quality(data)
        except Exception as e:
            print(f"[chip_provider] premium 取得失敗,降級 FinMind:{e}")
    try:
        data = _chips.get_chips(code) or {}
        return data, _quality(data)
    except Exception as e:
