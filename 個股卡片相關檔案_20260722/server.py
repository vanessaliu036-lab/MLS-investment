...
try:
    if opportunity_ledger_api.router is not None:
...
def api_dec_list(date: str = ""):
    ...
    _fresh_streak, _fresh_streak_date = {}, {}
    try:
        ...
        for _cd, _r in (_cc.get("stocks") or {}).items():
            if _r.get("inst_streak") is not None:
                ...
                # 帶上法人資料日：籌碼欄不標日期，前端就無從分辨這是哪一天的
                # 法人，AB 引擎那份停更時也看不出來。
                _fresh_streak_date[str(_cd)] = _r.get("source_date")
    except Exception:
        pass
    ...
    for w in wl:
        ...
        rows.append({
            ...
            "chip_streak": _fresh_streak.get(code),
            "chip_source_date": _fresh_streak_date.get(code),
        })
