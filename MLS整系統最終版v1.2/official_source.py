...
def latest_stock_institutional(code, now=None, lookback_days=8):
    """Return the latest published TWSE T86 row for ``code``.

    T86 is an end-of-day dataset, so on weekends, holidays, or before the
    current trading day's file is published, today's query legitimately has no
    row. Walk backward until the latest published trading day is found.
    ``None`` is returned when no official row is available in the lookback
    window; callers may then use an explicitly labelled fallback source.
    """
    anchor = now or datetime.now(TW_TZ)
    if anchor.tzinfo is None:
        anchor = anchor.replace(tzinfo=TW_TZ)
    else:
        anchor = anchor.astimezone(TW_TZ)
    for offset in range(max(1, int(lookback_days))):
        day = anchor - timedelta(days=offset)
        row = stock_institutional(code, date=day)
        if row and any(row.get(k) is not None for k in
